...
function showNotification(response) {
  ...
  if (response.status === 409) {
    ...
    if (response.data.providers.length === 1) {
      ...
      providersTextPart = `Please sign up using one of your ${response.data.providers.join(
        " or "
      )}`;
    }
    ...
    message =
      "Authentication provider has empty email. Please sign up using email and attach your social account in profile page to be able to use it for further sign in's";
  } else {
...
function openAuthenticationWindow(
  ...
  window.socialProviderCallback = function(provider, queryParams) {
    axios
      .get(backendHandlerUrl, { params: queryParams })
      .then((response) => {
